...
def create_amortization_schedule(request, pk):
    ...
    # HERE IS THE CRUCIAL CALL:
    generate_amortization_schedule(loan, amortization)
...
def generate_loan_report(request, loan_id):
    ...
    set_merged_cell_value(ws, "P11", loan.loan_amount)
    ws["R33"].value = loan.loan_amount
    set_merged_cell_value(ws, "R16", loan.term)
    set_merged_cell_value(ws, "O24", loan.loan_type)
    set_merged_cell_value(ws, "O26", "")
    ...
    for record in amortizations:
        ws.cell(row=current_row, column=1, value=record.seq)
        ws.cell(row=current_row, column=3, value=record.due_date)
        ws.cell(row=current_row, column=3).number_format = "DD-MM-YYYY"
        ws.cell(row=current_row, column=15, value=record.principal)
        ws.cell(row=current_row, column=16, value=record.interest)
        ws.cell(row=current_row, column=17, value=record.amortization)
        ws.cell(row=current_row, column=18, value=record.remaining_balance)
        current_row += 1
...
def last_backup_time(request):
    try:
        last_backup = BackupLog.objects.latest('backup_time')
...
def last_restore_time(request):
    try:
        last_restore = RestoreLog.objects.latest('timestamp')
        return Response({
